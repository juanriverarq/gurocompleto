...
export const PDF_CONFIG = {
  ...
  patterns: {
    ...
    primaNeta: /(?:prima\s*neta|valor\s*prima)\s*:?\s*\$?\s*([0-9,\.]+)/i,
    iva: /(?:iva|impuesto)\s*:?\s*\$?\s*([0-9,\.]+)/i,
    total: /(?:total|valor\s*total)\s*:?\s*\$?\s*([0-9,\.]+)/i,
    fechaExpedicion: /(?:fecha\s*expedición|fecha\s*expedicion|expedida|emitida|fecha\s*emisión|fecha\s*emision)\s*:?\s*([0-9]{1,2}\/[0-9]{1,2}\/[0-9]{4})/i,
    fechaInicio: /(?:fecha\s*inicio|vigencia\s*desde|desde|inicia)\s*:?\s*([0-9]{1,2}\/[0-9]{1,2}\/[0-9]{4})/i,
    fechaFin: /(?:fecha\s*fin|vigencia\s*hasta|hasta|vence|vencimiento)\s*:?\s*([0-9]{1,2}\/[0-9]{1,2}\/[0-9]{4})/i,
