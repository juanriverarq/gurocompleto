...
const useClienteValidation = () => {
  ...
  const validateStep = useCallback(
    (step: number, formData: ClienteFormData): boolean => {
      ...
      if (step === 1) {
        ...
        if (!formData.department || !formData.department.trim()) {
          stepErrors.department = 'El departamento es obligatorio';
          isValid = false;
        }
  ...
  const validateAll = useCallback(
    (formData: ClienteFormData): boolean => {
  ...
  const validateStepAndSetErrors = useCallback(
    (step: number, formData: ClienteFormData): boolean => {
