...
const DetalleLead: React.FC = () => {
  ...
  const confirmStateChange = async () => {
    ...
    try {
      ...
      const stateLabel =
        BUSINESS_STATES[newState as keyof typeof BUSINESS_STATES]?.label || newState;
      toast({
  ...
  const handleWhatsApp = () => {
    ...
    try {
      ...
      const message = encodeURIComponent(
        `Hola ${lead.first_name}, me comunico contigo para dar seguimiento a tu interés en nuestros seguros. ¿Cuándo podríamos conversar?`,
      );
  ...
  const stateConfig =
    BUSINESS_STATES[currentState as keyof typeof BUSINESS_STATES] || BUSINESS_STATES.nuevo;
      ...
      <div className="bg-white dark:bg-darkgray shadow-md dark:shadow-none rounded-[10px] p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                {/* <Badge
                  className={`${stateConfig.color} px-3 py-1 rounded-full text-sm font-medium`}
                >
          ...
          <div className="flex gap-2">
            <Button
              color="light"
              onClick={() => navigate('/apps/saas/sales-funnel')}
              className="rounded-[10px]"
            >
              <IconifyIcon icon="solar:arrow-left-bold" className="w-4 h-4 mr-2" />
              Volver
            </Button>
            <Button color="secondary" onClick={handleChangeState} className="rounded-[10px]">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {lead.updated_at
                    ? new Date(lead.updated_at).toLocaleDateString('es-CO')
                    : 'No disponible'}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-medium capitalize">
                  {lead.preferred_contact_method || 'No especificado'}
                </span>
              </div>
            </div>
          </div>
      ...
      <Card>
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Button
              color="light"
              className={`flex flex-col items-center p-4 h-auto rounded-[10px] ${
                !lead.phone ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-50'
              }`}
              onClick={handleCall}
              disabled={!lead.phone}
            ...
            <Button
              color="light"
              className={`flex flex-col items-center p-4 h-auto rounded-[10px] ${
                !lead.email ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-50'
              }`}
              onClick={handleEmail}
              disabled={!lead.email}
            ...
            >
              <IconifyIcon
                ...
                className="w-6 h-6 mb-2 text-orange-600"
              />
