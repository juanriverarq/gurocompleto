...
const MiPaginaWeb: React.FC = () => {
  const [viewMode, setViewMode] = useState<'gallery' | 'preview' | 'editor'>('gallery');
      ...
      <div className="p-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Elige una plantilla para tu sitio web</p>
          </div>
          {selectedTemplate && (
              ...
              <div className="relative bg-gray-100 dark:bg-gray-900 overflow-hidden" style={{ paddingBottom: '62.5%' }}
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setSelectedTemplate(t); setViewMode('editor'); }} className="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-medium shadow-lg">
    ...
    return createPortal(
      <div className="fixed inset-0 bg-white flex flex-col" style={{ zIndex: 2147483647 }}>
        <div className="h-12 bg-[#141414] flex items-center justify-between px-4 shrink-0">
          ...
          <button onClick={() => setViewMode('editor')} className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-xs font-medium"><Icon icon="solar:pen-new-square-bold" width={14} /> Usar plantilla</button>
        </div>
      ...
      <div className="fixed inset-0 flex flex-col" style={{ zIndex: 2147483647, background: '#0d0d0d' }}>
        ...
        <div className="h-11 bg-[#141414] border-b border-white/5 flex items-center justify-between px-3 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => sendToIframe('GET_HTML')} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-gray-300 rounded-md text-xs font-medium"><Icon icon="solar:diskette-bold" width={14} /> Guardar</button>
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-xs font-medium"><Icon icon="solar:global-bold" width={14} /> Publicar</button>
          </div>
        </div>
