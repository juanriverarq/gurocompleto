...
// ─── Persistence ─────────────────────────────────────────────────────
interface SavedImage { id: string; src: string; prompt: string; ts: number; }
...
type View = 'home' | 'generate' | 'inspire' | 'upload' | 'history';
...
const CreadorContenido = () => {
  ...
  const logo = tenant?.logo_url || tenant?.branding?.logo || (tenant as any)?.logo || null;
  const name = tenant?.branding?.nombre_comercial || (tenant as any)?.name || tenant?.nombre || '';
  const agencyColors = useMemo(() => {
    const c: string[] = []; const b = tenant?.branding;
    if (b?.primary_color) c.push(b.primary_color); else if (b?.colores?.primario) c.push(b.colores.primario);
    if (b?.secondary_color) c.push(b.secondary_color); else if (b?.colores?.secundario) c.push(b.colores.secundario);
    if (b?.accent_color) c.push(b.accent_color); else if (b?.colores?.acento) c.push(b.colores.acento);
  ...
  const save = () => {
    if (!canvas) return;
    const list = load(); list.unshift({ id: uid(), src: canvas, prompt: label, ts: Date.now() });
    if (list.length > 30) list.pop(); persist(list);
  };
  ...
  const generate = useCallback(async (promptOverride?: string, isEdit?: boolean) => {
    ...
    try {
      ...
      if (isEdit && canvas) {
        ...
        if (res.data[0]) {
          const src = `data:image/png;base64,${res.data[0].base64}`;
          setCanvas(src); setLabel(full); setEditPrompt('');
        }
      } else {
        // For new generation: use Flux Kontext Pro with URL if available (much better at following references)
        const res = await freepikService.generateImage({
          prompt: full, num_images: 1, image: { size: selectedSize },
  ...
  }, [genPrompt, refUrl, refBase64, canvas, name, agencyColors, selectedSize]);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] -mx-2 -mt-2 bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-white overflow-hidden rounded-2xl transition-colors duration-300">
      ...
      {canvas && (
        <div className="flex items-center justify-between px-4 h-10 bg-gray-50 dark:bg-[#111] border-b border-gray-200 dark:border-white/5 shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            <button onClick={save} className="px-3 py-1 rounded-lg bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-[10px] text-gray-600 dark:text-gray-300 transition-all active:scale-95">Guardar</button>
      ...
      <div className="flex flex-1 overflow-hidden">
                ...
                {[
                  ...
                  <FadeIn key={c.label} delay={c.delay}>
                    <button onClick={() => setView(c.v)}
                      ...
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 ${c.accent ? 'bg-purple-100 dark:bg-purple-500/10' : 'bg-gray-100 dark:bg-white/5'}`}>
                        <Icon icon={c.icon} width={19} className={c.accent ? 'text-purple-500 dark:text-purple-400' : 'text-gray-500 dark:text-gray-400'} />
                      </div>
        ...
        {view !== 'home' && sidebarView && !fullWidthView && (
          <>
            {/* Left panel */}
            <div className="w-[280px] bg-gray-50 dark:bg-[#111] border-r border-gray-200 dark:border-white/[0.04] overflow-y-auto shrink-0 transition-colors">
              <div className="p-4">
                <FadeIn>
                  ...
                  {view === 'history' && <HistoryPanel onSelect={(src, lbl) => { setCanvas(src); setLabel(lbl || ''); setView('generate'); }} />}
                </FadeIn>
              </div>
            </div>
...
const GeneratePanel = ({ prompt, setPrompt, loading, error, onGenerate, refImage, clearRef, hasRef, selectedSize, setSelectedSize }: {
      ...
      {!refImage && (
        <>
          ...
          <button onClick={onGenerate} disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 disabled:opacity-40 text-[11px] text-white font-bold transition-all active:scale-[0.97] shadow-lg shadow-purple-600/25">
            {loading ? <><Spin /> Generando...</> : <><Icon icon="solar:magic-stick-3-bold" width={14} /> Generar</>}
          </button>
...
// HISTORY PANEL
// ═════════════════════════════════════════════════════════════════════
const HistoryPanel = ({ onSelect }: { onSelect: (src: string, lbl?: string) => void }) => {
      ...
      {list.length === 0 ? (
        <FadeIn className="text-center py-10">
          <div className="w-12 h-12 rounded-2xl bg-gray-200 dark:bg-white/5 flex items-center justify-center mx-auto mb-3">
            <Icon icon="solar:folder-bold-duotone" width={22} className="text-gray-400 dark:text-gray-700" />
          </div>
          <p className="text-[11px] text-gray-400 dark:text-gray-700">Sin diseños guardados</p>
        </FadeIn>
      ) : (
          ...
          {list.map((p, i) => (
            <FadeIn key={p.id} delay={i * 40}>
              <div className="group relative rounded-xl overflow-hidden border border-gray-200 dark:border-white/[0.04] hover:border-purple-400 dark:hover:border-purple-500/20 transition-all duration-300 shadow-sm hover:shadow-lg">
                <button onClick={() => onSelect(p.src, p.prompt)} className="w-full">
                ...
                <button onClick={() => del(p.id)}
                  className="absolute top-1.5 right-1.5 w-5 h-5 rounded-lg bg-black/40 backdrop-blur flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500/80 active:scale-90">
