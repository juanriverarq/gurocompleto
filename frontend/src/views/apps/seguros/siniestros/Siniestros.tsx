...
import { Card, Button, Badge, Table, Modal, Tabs, Timeline, Progress, Spinner, Dropdown, Label, Checkbox } from 'flowbite-react';
...
import { siniestroService, Siniestro, TIPOS_SEGURO, TIPOS_SINIESTRO, ESTADOS, PRIORIDADES } from '../../../../services/siniestroService';
...
const Siniestros: React.FC = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>([
    'numero', 'cliente', 'poliza', 'tipo', 'estado', 'prioridad'
  ]);
  ...
  const loadSiniestros = async () => {
    try {
      ...
      setPagination({
        ...
        from: Number((response as any).from || ((params.page - 1) * params.per_page + ((response.data || []).length ? 1 : 0))),
        to: Number((response as any).to || ((params.page - 1) * params.per_page + (response.data || []).length)),
      });
  ...
  const availableColumns = [
    { key: 'numero', label: 'Número', description: 'Número de siniestro', icon: 'solar:document-text-bold', defaultVisible: true },
    { key: 'cliente', label: 'Cliente', description: 'Nombre del cliente', icon: 'solar:user-bold', defaultVisible: true },
    { key: 'poliza', label: 'Póliza', description: 'Número de póliza', icon: 'solar:document-bold', defaultVisible: true },
    { key: 'tipo', label: 'Tipo', description: 'Tipo de seguro', icon: 'solar:shield-check-bold', defaultVisible: true },
    { key: 'siniestro', label: 'Siniestro', description: 'Tipo de siniestro', icon: 'solar:danger-bold', defaultVisible: false },
    { key: 'estado', label: 'Estado', description: 'Estado actual', icon: 'solar:check-circle-bold', defaultVisible: true },
    { key: 'prioridad', label: 'Prioridad', description: 'Nivel de prioridad', icon: 'solar:flag-bold', defaultVisible: true },
    { key: 'fecha', label: 'Fecha Ocurrencia', description: 'Fecha del siniestro', icon: 'solar:calendar-bold', defaultVisible: false },
    { key: 'monto', label: 'Monto', description: 'Monto reclamado', icon: 'solar:dollar-bold', defaultVisible: false },
    { key: 'progreso', label: 'Progreso', description: 'Progreso del siniestro', icon: 'solar:graph-bold', defaultVisible: false }
  ];
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-lg font-bold text-indigo-600">{formatCurrency(estadisticas.montoAprobado)}</p>
            </div>
          </div>
        </Card>
      </div>
          ...
          <>
            ...
            <div className="hidden lg:block overflow-x-auto table-container-with-dropdowns">
              <Table hoverable>
                <Table.Head>
                    ...
                    <Table.HeadCell>
                      <div className={`flex items-center gap-1 ${columnToApiField.numero ? 'cursor-pointer select-none' : ''}`} onClick={() => toggleSort('numero')}>
                        ...
                        {filters.sort_by === columnToApiField.numero && (
                          <Icon icon={filters.sort_dir === 'asc' ? 'solar:arrow-up-bold-duotone' : 'solar:arrow-down-bold-duotone'} className="w-4 h-4 text-gray-400" />
                        )}
                      </div>
                    </Table.HeadCell>
                  )}
                  ...
                  {visibleColumns.includes('progreso') && <Table.HeadCell>Progreso</Table.HeadCell>}
                  ...
                  {paginatedSiniestros.map((siniestro) => (
                    <Table.Row key={siniestro.id} className="">
                      ...
                      {visibleColumns.includes('cliente') && (
                        <Table.Cell className="whitespace-nowrap">{getClienteNombre(siniestro)}</Table.Cell>
                      )}
                      {visibleColumns.includes('poliza') && (
                        <Table.Cell className="whitespace-nowrap">{siniestro.numero_poliza}</Table.Cell>
                      )}
                      {visibleColumns.includes('tipo') && (
                      ...
                      {visibleColumns.includes('siniestro') && (
                        <Table.Cell className="whitespace-nowrap">{(TIPOS_SINIESTRO as any)[siniestro.tipo_siniestro] || siniestro.tipo_siniestro}</Table.Cell>
                      )}
                      {visibleColumns.includes('estado') && (
                        <Table.Cell className="whitespace-nowrap">
                      ...
                      {visibleColumns.includes('fecha') && (
                        <Table.Cell className="whitespace-nowrap">{new Date(siniestro.fecha_ocurrencia).toLocaleDateString('es-CO')}</Table.Cell>
                      )}
                      {visibleColumns.includes('monto') && (
                        <Table.Cell className="whitespace-nowrap">{formatCurrency(siniestro.monto_reclamado)}</Table.Cell>
                      )}
                ...
                <Card key={siniestro.id} className="p-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-sm font-medium">{formatCurrency(siniestro.monto_reclamado)}</p>
                    </div>
                    <div>
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex items-center gap-2 text-sm text-gray-500">
                      ...
                      <span>Reportado: {new Date(siniestro.fecha_reporte).toLocaleDateString('es-CO')}</span>
                    </div>
                    <Dropdown
                      label=""
                      dismissOnClick={false}
            ...
            <div className="flex items-center justify-between p-4 border-t">
              <div className="text-sm text-gray-600">
                Mostrando {pagination.from || 0} a {pagination.to || 0} de {pagination.total || 0} siniestros
              </div>
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  onClick={() => handlePageChange(Math.min((pagination.last_page || 1), (pagination.current_page || 1) + 1))}
                  className="rounded-[10px]"
                >
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{(TIPOS_SINIESTRO as any)[selectedSiniestro.tipo_siniestro] || selectedSiniestro.tipo_siniestro}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Estado:</span>
                        <Badge color={getEstadoBadge(selectedSiniestro.estado)} className="capitalize">
                          {(ESTADOS as any)[selectedSiniestro.estado] || selectedSiniestro.estado.replace('_', ' ')}
                        </Badge>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Prioridad:</span>
                        <Badge color={getPrioridadBadge(selectedSiniestro.prioridad)} className="capitalize">
                          {(PRIORIDADES as any)[selectedSiniestro.prioridad] || selectedSiniestro.prioridad}
                        </Badge>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Fecha Ocurrencia:</span>
                        <span className="font-medium">{new Date(selectedSiniestro.fecha_ocurrencia).toLocaleDateString('es-CO')}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Fecha Reporte:</span>
                        <span className="font-medium">{new Date(selectedSiniestro.fecha_reporte).toLocaleDateString('es-CO')}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{formatCurrency(selectedSiniestro.monto_reclamado)}</span>
                      </div>
                      {selectedSiniestro.monto_aprobado && (
                        <div className="flex justify-between">
                          <span className="text-gray-600">Monto Aprobado:</span>
                          <span className="font-medium">{formatCurrency(selectedSiniestro.monto_aprobado)}</span>
                        </div>
                      )}
                      <div className="flex justify-between">
                      ...
                      {selectedSiniestro.assigned_adjuster && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{selectedSiniestro.assigned_adjuster.name}</span>
                        </div>
                      )}
                      <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{selectedSiniestro.dias_tramite || 0} días</span>
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedSiniestro.cliente?.mobile_phone && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{selectedSiniestro.cliente.mobile_phone}</span>
                        </div>
                      )}
                      {selectedSiniestro.cliente?.document_number && (
                        <div className="flex justify-between">
                          <span className="text-gray-600">Documento:</span>
                          <span className="font-medium">{selectedSiniestro.cliente.document_type} {selectedSiniestro.cliente.document_number}</span>
                        </div>
                      )}
                      {selectedSiniestro.cliente?.birth_date && (
                        <div className="flex justify-between">
                          <span className="text-gray-600">Fecha Nacimiento:</span>
                          <span className="font-medium">{new Date(selectedSiniestro.cliente.birth_date).toLocaleDateString('es-CO')}</span>
                        </div>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedSiniestro.cliente?.emergency_contact_name && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{selectedSiniestro.cliente.emergency_contact_name}</span>
                        </div>
                      )}
                      {selectedSiniestro.cliente?.emergency_contact_phone && (
                        <div className="flex justify-between">
                          <span className="text-gray-600">Tel. Emergencia:</span>
                          <span className="font-medium">{selectedSiniestro.cliente.emergency_contact_phone}</span>
                        </div>
                      )}
                    </div>
                  </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {selectedSiniestro.cliente?.occupation && (
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{selectedSiniestro.cliente.occupation}</span>
                          </div>
                        )}
                      </div>
                      <div className="space-y-2">
                        {selectedSiniestro.cliente?.work_address && (
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">{selectedSiniestro.cliente.work_address}</span>
                          </div>
                        )}
                        {selectedSiniestro.cliente?.monthly_income && (
                          <div className="flex justify-between">
                            <span className="text-gray-600">Ingresos Mensuales:</span>
                            <span className="font-medium">{formatCurrency(selectedSiniestro.cliente.monthly_income)}</span>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <Timeline>
                  <Timeline.Item>
                    ...
                    <Timeline.Content>
                      ...
                      <Timeline.Body>
                        El siniestro fue reportado por el cliente {getClienteNombre(selectedSiniestro)}
                      </Timeline.Body>
                    </Timeline.Content>
                  </Timeline.Item>
                  ...
                  {['aprobado', 'pagado'].includes(selectedSiniestro.estado) && (
                    <Timeline.Item>
                      ...
                      <Timeline.Content>
                        ...
                        <Timeline.Body>
                          El siniestro ha sido aprobado por un monto de {formatCurrency(selectedSiniestro.monto_aprobado || 0)}
                        </Timeline.Body>
                      </Timeline.Content>
                    </Timeline.Item>
                  )}
                  ...
                  {selectedSiniestro.estado === 'rechazado' && (
                    <Timeline.Item>
                      ...
                      <Timeline.Content>
                        ...
                        <Timeline.Body>
                          El siniestro ha sido rechazado
                        </Timeline.Body>
                      </Timeline.Content>
                    </Timeline.Item>
                  )}
      ...
      <Modal show={showFilterModal} onClose={() => setShowFilterModal(false)} size="lg">
        ...
        <Modal.Body>
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label className="text-gray-900 dark:text-white mb-2 block font-medium">Tipo de Seguro</Label>
              ...
              <div>
                <Label className="text-gray-900 dark:text-white mb-2 block font-medium">Estado</Label>
              ...
              <div>
                <Label className="text-gray-900 dark:text-white mb-2 block font-medium">Prioridad</Label>
                <Select value={filters.prioridad} onValueChange={(value) => setFilters(prev => ({ ...prev, prioridad: value }))}>
                  ...
                  <SelectContent>
                    ...
                    {prioridadesSiniestro.map(prioridad => (
                      <SelectItem key={prioridad.value} value={prioridad.value}>{prioridad.label}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600 dark:text-gray-400">Filtros activos: {getActiveFiltersCount()}</span>
              </div>
          ...
          {selectedSiniestro && (
            <div className="space-y-4">
              <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                <h4 className="font-semibold text-gray-900 dark:text-white mb-2">Siniestro: {selectedSiniestro.numero_siniestro}</h4>
              ...
              <div>
                <Label className="text-gray-900 dark:text-white mb-2 block font-medium">Nuevo Estado</Label>
              ...
              <div>
                <Label className="text-gray-900 dark:text-white mb-2 block font-medium">Observación (Opcional)</Label>
                <textarea
              ...
              {newState && (
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3">
                  <div className="flex items-center gap-2 text-sm text-blue-900 dark:text-blue-200">
                    ...
                    <span>El estado cambiará de <strong>{(ESTADOS as any)[selectedSiniestro.estado] || selectedSiniestro.estado.replace('_', ' ')}</strong> a <strong>{(ESTADOS as any)[newState] || newState.replace('_', ' ')}</strong></span>
                  </div>
                </div>
              )}
            </div>
