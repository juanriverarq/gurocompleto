...
const ArchivosPoliza: React.FC<Props> = ({ polizaId }) => {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await polizaService.listarDocumentos(polizaId);
      if (res.success) {
        setItems(res.data || []);
      }
    } finally {
  ...
  const onDelete = async (doc: any) => {
    try {
      await polizaService.eliminarDocumento(polizaId, { path: doc.path });
      await load();
    } catch {}
  };
  ...
  return (
    <Card>
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="min-w-[240px] flex-1">
            <TextInput
              ...
              placeholder="Buscar por nombre o tipo MIME"
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            <Dropdown label={`Orden: ${sortBy} · ${sortDir === 'asc' ? '↑' : '↓'}`} inline>
              ...
              <Dropdown.Divider />
              <Dropdown.Item onClick={() => setSortDir(sortDir === 'asc' ? 'desc' : 'asc')}>
                Dirección: {sortDir === 'asc' ? 'Ascendente' : 'Descendente'}
              </Dropdown.Item>
            </Dropdown>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              const url = await polizaService.getSignedUrl(polizaId, { path: doc.path, name: doc.name });
      ...
      {/* Modal para seleccionar tipo y subir archivo */}
        ...
        <Modal.Body>
          <div className="space-y-4">
            ...
            <div>
              ...
              {/* El dropzone también abre el diálogo al hacer clic */}
