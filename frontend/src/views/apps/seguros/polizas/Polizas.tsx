...
import { Card, Button, Badge, Table, Modal, Tabs, Spinner, Dropdown, Checkbox } from 'flowbite-react';
...
const { toast } = useToast();
  ...
  const tiposSeguro = [
    ...
    { value: 'responsabilidad_civil', label: 'Responsabilidad Civil', icon: 'solar:shield-check-bold-duotone', color: 'indigo' },
    { value: 'todo_riesgo', label: 'Todo Riesgo', icon: 'solar:shield-star-bold-duotone', color: 'cyan' },
    { value: 'incendio', label: 'Incendio', icon: 'solar:fire-bold-duotone', color: 'orange' },
    { value: 'transporte', label: 'Transporte', icon: 'solar:delivery-bold-duotone', color: 'teal' },
    { value: 'accidentes', label: 'Accidentes Personales', icon: 'solar:medical-kit-bold-duotone', color: 'rose' }
  ];
  ...
  const handleViewPoliza = (poliza: Poliza) => {
    ...
    const isAuto = String((poliza as any).ramo_principal || (poliza as any).ramo_nombre || '').toLowerCase().includes('auto');
    if (isAuto || hasPlacas) {
      setAutosTabLoading(true);
      saasApi.getAutomoviles({ poliza_id: poliza.id, per_page: 50 })
        .then((resp) => {
...
const isISODateOnly = (s: any): boolean =>
...
const formatDate = (value: any): string => {
  ...
  const renderTableCell = (poliza: Poliza, columnKey: string) => {
    switch (columnKey) {
      ...
      case 'cliente':
        return (
          <div>
            <div className="font-medium uppercase">{poliza.nombres_cliente} {poliza.apellidos_cliente}</div>
      ...
      case 'ramo':
        return (
          <div className="flex items-center gap-2">
            {getTipoIcon(poliza.ramo_principal)}
            <span className="uppercase">{(poliza as any).ramo_nombre || (tiposSeguro.find(t => t.value === poliza.ramo_principal)?.label || poliza.ramo_principal)}</span>
          </div>
        );
      case 'estado':
        return (
  ...
  const toggleSelectAll = () => {
    if (isAllSelected) {
      ...
      polizas.forEach((p) => { if (p.id) next.add(String(p.id)); });
      setSelectedIds(next);
    }
  };
  ...
  const handleBulkDeletePolizas = async () => {
    ...
    if (!confirm(`¿Eliminar ${selectedIds.size} póliza(s) seleccionadas? Esta acción no se puede deshacer.`)) return;
    try {
      setLoading(true);
  ...
  const handleConfirmBulkStateChange = async () => {
    ...
    try {
      setLoading(true);
      const ids = Array.from(selectedIds);
      const results = await Promise.allSettled(ids.map((id) => polizaService.cambiarEstado(id, bulkTargetState as any)));
      const ok = results.filter((r) => r.status === 'fulfilled').length;
      const fail = results.length - ok;
      toast({
  ...
  return (
    <OnboardingGuard>
      <PermissionGate route="/apps/seguros/polizas" action="ver" fallback={<div className="p-6 text-center text-gray-500">No tienes permisos para ver Pólizas.</div>}>
            ...
            <Card className="p-3 md:p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg md:text-2xl font-bold text-orange-600">{estadisticas.polizas_por_vencer}</p>
                </div>
            ...
            <Card className="p-3 md:p-4 col-span-2 sm:col-span-3 md:col-span-1">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-lg md:text-2xl font-bold text-red-600">{estadisticas.polizas_vencidas}</p>
                </div>
              ...
              <div className="hidden lg:block overflow-x-auto table-container-with-dropdowns">
                <Table hoverable>
                  ...
                  <Table.Body className="">
                    {polizas && polizas.length > 0 && polizas.map((poliza) => (
                  ...
                  <Card key={poliza.id} className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{(poliza as any).aseguradora_nombre || poliza.aseguradora}</p>
                        </div>
                      </div>
                      <Badge color={getEstadoBadge(poliza.estado || 'ACTIVA')}>
                        {estadosPoliza.find(e => e.value === poliza.estado)?.label || poliza.estado}
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-3">
                      <div>
                        ...
                        <p className="text-sm font-medium uppercase">{poliza.nombres_cliente} {poliza.apellidos_cliente}</p>
      ...
      <Modal show={showCreateTypeModal} onClose={() => setShowCreateTypeModal(false)} size="md">
                    ...
                    <div>
                      ...
                      <div className="space-y-3 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span>{selectedPoliza.nombres_cliente} {selectedPoliza.apellidos_cliente}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          <strong>Email:</strong> 
                          <span className="text-right max-w-48 truncate">{selectedPoliza.correo_cliente}</span>
                        </div>
                        <div className="flex justify-between">
                          <strong>Dirección:</strong> 
                          <span className="text-right max-w-48 truncate">{selectedPoliza.domicilio}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between"><strong>Renovable:</strong><span>{selectedPoliza.renovable ? 'Sí' : 'No'}</span></div>
                        <div className="flex justify-between"><strong>Motivo:</strong><span className="text-right max-w-48 truncate">{selectedPoliza.motivo || '-'}</span></div>
                         <div className="flex justify-between"><strong>Pri a Pre:</strong><span>{selectedPoliza.pri_a_pre ?? '-'}</span></div>
                        <div className="flex justify-between"><strong>Participación:</strong><span>{selectedPoliza.participacion ?? '-'}</span></div>
                        <div className="flex justify-between"><strong>Co‑corretaje:</strong><span>{selectedPoliza.co_corretaje ?? '-'}</span></div>
                        <div className="flex justify-between"><strong>Comisión Agencia:</strong><span>{selectedPoliza.comision_agencia ?? '-'}</span></div>
                        <div className="flex justify-between"><strong>% Retención:</strong><span>{selectedPoliza.porcentaje_retencion ?? '-'}</span></div>
                        <div className="flex justify-between"><strong>% Reteiva:</strong><span>{selectedPoliza.porcentaje_reteiva ?? '-'}</span></div>
                      </div>
                    </div>
                    <div>
                      <h4 className="font-semibold mb-2">Beneficiarios</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between"><strong>En remisión:</strong><span>{selectedPoliza.beneficiario_en_remision ? 'Sí' : 'No'}</span></div>
                        <div className="flex justify-between"><strong>Oneroso - Nombre:</strong><span className="text-right max-w-48 truncate">{selectedPoliza.beneficiario_oneroso_nombre || '-'}</span></div>
                        <div className="flex justify-between"><strong>Oneroso - Documento:</strong><span>{selectedPoliza.beneficiario_oneroso_documento || '-'}</span></div>
                      </div>
                    </div>
                    <div>
                      <h4 className="font-semibold mb-2">Otros</h4>
                      <div className="space-y-2">
                        <div className="flex justify-between"><strong>Fecha Recepción:</strong><span>{selectedPoliza.fecha_recepcion ? formatDate(selectedPoliza.fecha_recepcion) : '-'}</span></div>
                         
                        <div className="flex justify-between"><strong>Banco:</strong><span>{(selectedPoliza as any).bank_name || '-'}</span></div>
                        <div className="flex justify-between"><strong># Cuotas:</strong><span>{(selectedPoliza as any).installments_count ?? '-'}</span></div>
                        <div className="flex justify-between"><strong>Tarjeta:</strong><span>{(selectedPoliza as any).card_last4 ? `**** **** **** ${(selectedPoliza as any).card_last4}` : '-'}</span></div>
                         
                      </div>
                    </div>
                  </div>
                </Tabs.Item>
                {(String(((selectedPoliza as any).ramo_principal || (selectedPoliza as any).ramo_nombre) || '').toLowerCase().includes('auto') || (Array.isArray((selectedPoliza as any).placas) && (selectedPoliza as any).placas.length > 0)) && (
                  <Tabs.Item title="Automóviles">
                    {autosTabLoading ? (
                      <div className="flex items-center gap-2"><Spinner size="sm" /> Cargando automóviles...</div>
                    ) : autosVinculados.length === 0 ? (
                      <div className="text-sm text-gray-500">No hay automóviles vinculados a esta póliza.</div>
                    ) : (
                          ...
                          <Table.Body>
                            {autosVinculados.map((a: any) => (
                              <Table.Row key={a.id}>
                                ...
                                <Table.Cell>{polizaUtils.formatCurrency((a as any).insured_value || 0)}</Table.Cell>
                                <Table.Cell>
                                  <Link to={`/apps/seguros/automoviles?search=${encodeURIComponent(a.placa || '')}`}>
                                    <Button size="xs" color="light">Editar en Automóviles</Button>
                                  </Link>
                                </Table.Cell>
                              </Table.Row>
                            ))}
        ...
        <Modal show={showBulkStateModal} onClose={() => setShowBulkStateModal(false)} size="md">
          <Modal.Header>
            Cambiar estado a {selectedIds.size} póliza(s)
          </Modal.Header>
          <Modal.Body>
            <div className="space-y-3">
