...
const BCrumb = [
  { to: '/', title: 'Inicio' },
  { title: 'Gestión Documental - Internos' },
];

const DocumentosInternos: React.FC = () => {
  ...
  const loadAllDocs = useCallback(async () => {
    ...
    setErrorDocs(null);
    try {
      const res = await internalDocumentsService.listarDocumentosGlobal();
  ...
  const typesOptions = useMemo(() => {
    ...
    // Base sugerida para internos
    const base = ['procedimiento', 'politica', 'formato', 'acta', 'contrato', 'manual', 'certificado', 'otro'];
  ...
  const visibleDocs = useMemo(() => {
    ...
    let list = docs.filter((d) => {
      ...
      const okSearch =
        ...
        String(d.contentType || '').toLowerCase().includes(s);
  ...
  const onOpen = async (row: any) => {
    try {
      setOpeningPath(row.path);
      const url = await internalDocumentsService.getSignedUrl({ path: row.path });
  ...
  const onDelete = async (row: any) => {
    await internalDocumentsService.eliminarDocumento({ path: row.path });
    await loadAllDocs();
  };
  ...
  return (
    <>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Gestiona documentos internos de la organización
            </p>
          </div>
      ...
      <Modal show={showUpload} onClose={() => setShowUpload(false)} size="lg">
        <Modal.Header>Subir documentos internos</Modal.Header>
        <Modal.Body>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                await internalDocumentsService.subirDocumento(
                  selectedFiles,
                  { type: docType },
...
export default DocumentosInternos;
