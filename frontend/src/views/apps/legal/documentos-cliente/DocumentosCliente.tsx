...
const DocumentosCliente: React.FC = () => {
  ...
  const [uploadClienteId, setUploadClienteId] = useState<string>('');
  const [docType, setDocType] = useState<string>('otros');
    ...
    (async () => {
      try {
        ...
        if (res.success && Array.isArray(res.data)) {
          ...
          setClientes([]);
          setErrorClientes(res.message || 'No se pudieron cargar los clientes');
        }
      } catch (e) {
        if (mounted) setErrorClientes(e instanceof Error ? e.message : 'Error desconocido');
      } finally {
  ...
  const buildClienteLabel = useCallback((c: Cliente): string => {
    if (c.razon_social && c.razon_social.trim().length > 0) {
      return `${c.razon_social}${c.cuit ? ` · ${c.cuit}` : ''}`;
    }
    const fullName = [c.nombre, c.apellidos].filter(Boolean).join(' ').trim();
  ...
  const loadAllDocs = useCallback(async () => {
    ...
    setErrorDocs(null);
    try {
      // Nueva estrategia: una sola petición global (rápido como pólizas)
      const res = await clienteDocumentsService.listarDocumentosGlobal({ per_page: 1000, page: 1 });
      const list: any[] = res && res.success && Array.isArray(res.data) ? res.data : [];
      const docsWithLabels = list.map((d: any) => {
        const label =
          (d.cliente_nombre ? String(d.cliente_nombre) : '') +
          (d.cliente_cuit ? ` · ${d.cliente_cuit}` : '');
        return {
          ...d,
          __clienteId: String(d.cliente_id ?? d.id ?? ''),
          __clienteLabel: label.trim() || String(d.cliente_id ?? ''),
        };
      });
  ...
  // Cargar documentos inmediatamente (sin esperar lista de clientes)
  useEffect(() => {
    loadAllDocs();
  }, [loadAllDocs]);
  ...
  const onDelete = async (row: any) => {
    await clienteDocumentsService.eliminarDocumento(String(row.__clienteId), { path: row.path });
    await loadAllDocs();
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Gestiona documentos de todos los clientes
            </p>
          </div>
              ...
              <Table hoverable>
                ...
                <Table.Body>
                  {paginatedDocs.map((d, idx) => (
                    <Table.Row key={`${d.__clienteId}:${d.path}:${idx}`}>
                      <Table.Cell className="font-medium text-gray-900 dark:text-white">
                        <div className="max-w-xs truncate" title={d.__clienteLabel}>
                          {d.__clienteLabel}
                        </div>
                      </Table.Cell>
                      <Table.Cell>
