...
export const internalDocumentsService = {
  ...
  async getSignedUrl(params: { path: string }): Promise<string> {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/saas/internal-documents/signed-url?${query}`, { headers });
    ...
    try {
      const streamRes = await fetch(`${API_BASE_URL}/saas/internal-documents/stream?${query}`, { headers });
      if (streamRes.ok) {
