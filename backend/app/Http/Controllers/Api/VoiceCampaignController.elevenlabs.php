...
class VoiceCampaignController extends Controller
{
    ...
    private function makeElevenLabsCall(
        ...
        try {
            ...
            // Detectar si el voice_message_template es un prompt completo del agente
            $isFullAgentPrompt = (strlen($message) > 500 && (
                str_contains($message, '# IDENTIDAD') ||
                str_contains($message, '# FLUJO') ||
                str_contains($message, '# REGLAS') ||
                str_contains($message, '# Personality') ||
                str_contains($message, '## APERTURA')
            ));

            if ($isFullAgentPrompt) {
                $finalPrompt = $message;
                $personalizedFirstMessage = "Hola {$customerName}, soy {$agentDisplayName} de {$safeCompany}. ¿Tienes un momento?";
            } else {
