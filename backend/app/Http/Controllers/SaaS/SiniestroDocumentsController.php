    ...
    public function stream(Request $request, int $id)
    {
        try {
            ...
            if (!$brokerId) {
                $user = Auth::user();
                if (!$user || !$user->broker_id) {
                    return response()->json(['success' => false, 'message' => 'Usuario no tiene un broker asignado'], 403);
                }
                $brokerId = $user->broker_id;
            }
            $siniestro = Siniestro::where('broker_id', $brokerId)->where('id', $id)->first();
            ...
            $bucket = $this->getBucket();
            return $this->streamFromBucketOrFallback($bucket, $doc, (int) $brokerId);
        } catch (AuthenticationException $ae) {
            return response()->json(['success' => false, 'message' => 'No autenticado'], 401);
        } catch (\Throwable $e) {
            Log::error('[SINIESTROS_STREAM] Error', ['siniestro_id' => $id, 'error' => $e->getMessage()]);
